...
const WORLDS = [
  { year: 2011, team: 'Fnatic', short: 'FNC', region: 'EU', accent: '#f1a23c', skins: [
  ...
  { year: 2012, team: 'Taipei Assassins', short: 'TPA', region: 'TW', accent: '#d4383d', skins: [
  ...
  { year: 2013, team: 'SK Telecom T1', short: 'SKT', region: 'KR', accent: '#e64242', skins: [
    ['Jax','Jax','SKT T1 Jax'],['Lee Sin','LeeSin','SKT T1 Lee Sin'],['Zed','Zed','SKT T1 Zed'],['Vayne','Vayne','SKT T1 Vayne'],['Zyra','Zyra','SKT T1 Zyra']
  ]},
  { year: 2014, team: 'Samsung White', short: 'SSW', region: 'KR', accent: '#dde8f5', skins: [
  ...
  { year: 2015, team: 'SK Telecom T1', short: 'SKT', region: 'KR', accent: '#e64242', skins: [
  ...
  { year: 2016, team: 'SK Telecom T1', short: 'SKT', region: 'KR', accent: '#e64242', skins: [
  ...
  { year: 2017, team: 'Samsung Galaxy', short: 'SSG', region: 'KR', accent: '#6d8fd7', skins: [
  ...
  { year: 2018, team: 'Invictus Gaming', short: 'IG', region: 'CN', accent: '#e4e8ee', skins: [
  ...
  { year: 2019, team: 'FunPlus Phoenix', short: 'FPX', region: 'CN', accent: '#ee7440', skins: [
  ...
  { year: 2020, team: 'DAMWON Gaming', short: 'DWG', region: 'KR', accent: '#55aee8', skins: [
  ...
  { year: 2021, team: 'Edward Gaming', short: 'EDG', region: 'CN', accent: '#cdd9e9', skins: [
    ['Graves','Graves','EDG Graves'],['Viego','Viego','EDG Viego'],['Zoe','Zoe','EDG Zoe'],['Aphelios','Aphelios','EDG Aphelios'],['Yuumi','Yuumi','EDG Yuumi']
  ]},
  { year: 2022, team: 'DRX', short: 'DRX', region: 'KR', accent: '#86b7ff', skins: [
  ...
  { year: 2023, team: 'T1', short: 'T1', region: 'KR', accent: '#e64c4c', skins: [
  ...
  { year: 2024, team: 'T1', short: 'T1', region: 'KR', accent: '#e64c4c', skins: [
  ...
  { year: 2025, team: 'T1', short: 'T1', region: 'KR', accent: '#f05b5b', skins: [
...
const COPY = {
  vi: {
    ...
    editions: 'Mùa CKTG', dynasties: 'Đội từng vô địch', record: 'Kỷ lục danh hiệu', skins: 'Skin vinh danh',
...
async function resolveSkin(entry) {
  ...
  try {
    ...
    let match = skins.find(skin => normalize(skin.name) === target);
    if (!match) match = skins.find(skin => normalize(skin.name).includes(target) || target.includes(normalize(skin.name)));
    if (!match) {
      const tokens = normalize(wanted.replace(champion, ''));
      match = skins.find(skin => tokens && normalize(skin.name).includes(tokens));
    }
    if (!match) return { url: baseSplash(id), found: false, actual: wanted };
    return { url: `https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${encodeURIComponent(id)}_${match.num}.jpg`, found: true, actual: match.name };
  } catch {
...
function titleCount(row) {
  return WORLDS.filter(x => x.year <= row.year && x.team === row.team).length;
}

function uniqueChampions() { return new Set(WORLDS.map(x => x.team)).size; }
...
function timelineHtml() {
  return `<div class="worlds-years" role="tablist" aria-label="World Championship years">${[...WORLDS].reverse().map(row => `
    <button type="button" role="tab" class="world-year ${row.year === activeYear ? 'active' : ''}" data-world-year="${row.year}" style="--team-accent:${row.accent}">
