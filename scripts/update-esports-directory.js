...
async function downloadCsvUrl(url, label) {
  ...
  if (text.length < 100_000 || !/playername/i.test(text.slice(0, 5000))) throw new Error(`${label}: response không giống CSV esports hợp lệ.`);
...
async function downloadOracleCsv() {
  ...
  for (let daysAgo = 0; daysAgo <= 14; daysAgo++) {
    ...
    try { return { text: await downloadCsvUrl(url, `official snapshot ${stamp}`), stamp, url, sourceMode: 'official-s3' }; }
    catch (error) { lastError = error; if (!/HTTP 404/.test(error.message)) console.log(error.message); }
  }
  ...
  for (const url of ORACLE_MIRRORS) {
    try { return { text: await downloadCsvUrl(url, new URL(url).hostname), stamp: `mirror-${ymd(now)}`, url, sourceMode: 'oracle-mirror' }; }
    catch (error) { lastError = error; console.log(error.message); }
  }
...
function buildDirectoryFromOracle(text, source) {
  ...
  const first = (row, names) => { for (const name of names) { const value = get(row, name); if (value !== '' && value != null) return value; } return ''; };
  for (const col of ['playername', 'teamname', 'position', 'champion', 'date']) if (!index.has(col)) throw new Error(`Oracle CSV thiếu cột ${col}.`);
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (gameId && !p.games.has(gameId)) {
      p.games.add(gameId); if (result === 1) p.wins++; p.kills += kills; p.deaths += deaths; p.assists += assists; p.cs += cs; p.dpm += dpm; p.visionScore += visionScore; p.statRows++;
    }
    if (!p.latestAt || date > p.latestAt) {
      p.latestAt = date; p.latestPatch = patch || p.latestPatch;
      p.team = { id: teamId, name: teamName, short: null, region: league, location: null, logo: null, sourcePage: wikiUrl(teamName), website: null, socials: {} };
    }
    const existingTeam = teams.get(teamId);
    if (!existingTeam || date > existingTeam.latestAt) teams.set(teamId, { id: teamId, name: teamName, short: null, region: league, location: null, logo: null, sourcePage: wikiUrl(teamName), website: null, socials: {}, latestAt: date });
  }

  const playerList = [...players.values()].filter(p => p.team && p.games.size > 0).map(p => {
    const championPool = [...p.champions.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 8);
  ...
  const teamList = [...teams.values()].filter(t => activeTeamIds.has(t.id)).map(({ latestAt, ...team }) => team).sort((a, b) => (a.region || '').localeCompare(b.region || '') || a.name.localeCompare(b.name));
  const regions = [...new Set(playerList.map(p => p.team?.region).filter(Boolean))].sort();
  return {
    generatedAt: new Date().toISOString(), source: "Oracle's Elixir professional match data", sourceType: source.sourceMode === 'official-s3' ? 'Oracle official downloadable dataset' : "mirror of Oracle's Elixir dataset",
    licenseNote: "Game statistics are property of Riot Games; directory rows are derived from Oracle's Elixir match data.",
    coverage: `Tuyển thủ xuất hiện trong dataset Oracle's Elixir • ${playerList.length} tuyển thủ • ${teamList.length} đội.`, sourceSnapshot: source.stamp, sourceUrl: source.url, sourceMode: source.sourceMode,
    playerCount: playerList.length, teamCount: teamList.length, regions, countries: [], teams: teamList, players: playerList
  };
}

function yearFromDate(value) { return String(value || '').match(/20\d{2}/)?.[0] || new Date().getUTCFullYear(); }
...
if (result.playerCount < 50 || result.teamCount < 10) throw new Error(`Directory quá nhỏ: ${result.playerCount} tuyển thủ / ${result.teamCount} đội. Không ghi đè dữ liệu cũ.`);
