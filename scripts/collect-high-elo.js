...
async function riot(url, retry = 0) {
  ...
  try {
    response = await fetch(url, {
      headers: {
        'X-Riot-Token': key,
        'User-Agent': 'WebLienMinh/2.0 (+GitHub Actions)'
      }
    });
  } catch (error) {
    if (retry < 5) {
      const wait = Math.min(30_000, 2_500 * (retry + 1));
  ...
  if ([500, 502, 503, 504].includes(response.status) && retry < 6) {
    const wait = Math.min(35_000, 3_000 * (retry + 1));
    console.log(`Riot ${response.status} tạm thời. Chờ ${wait / 1000}s rồi thử lại...`);
    await sleep(wait);
    return riot(url, retry + 1);
  }
...
const league = await riot(`https://${platform}.api.riotgames.com/lol/league/v4/challengerleagues/by-queue/RANKED_SOLO_5x5`);
...
for (const entry of entries) {
  if (entry.puuid) { puuids.push(entry.puuid); continue; }
...
for (const matchId of matchIds) {
  ...
  if (currentPatch && !gameVersion.startsWith(`${currentPatch}.`)) { skippedPatch++; continue; }
  await fs.writeFile(path.join(outDir, `${matchId}.json`), JSON.stringify(match));
  saved++;
...
if (!saved) throw new Error(`Chưa lấy được trận Ranked Solo/Duo thuộc patch ${currentPatch}. Có thể patch vừa phát hành và Challenger chưa có đủ trận mới.`);
